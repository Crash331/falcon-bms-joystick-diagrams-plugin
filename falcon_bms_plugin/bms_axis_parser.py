...
_SETUP_PROFILE_RE = re.compile(
    r"^Setup\.v\d+\.(?P<name>.+?)\s+\{[0-9a-f-]+\}\.xml$", re.IGNORECASE
)
...
class BMSAxisParser:
    """Parser for active axes in a Falcon BMS User\\Config directory."""
    ...
    def parse(self) -> list[ParsedAxisBinding]:
        ...
        active_names = {name.casefold() for name in active_devices}
        setup_files: dict[str, tuple[str, Path]] = {}
        ...
        for path in self.config_dir.glob("Setup.v*.xml"):
            ...
            if active_names and normalised not in active_names:
                continue
            existing = setup_files.get(normalised)
            if existing is None or path.stat().st_mtime > existing[1].stat().st_mtime:
                setup_files[normalised] = (device_name, path)

        if active_devices:
            ordered_profiles = [
                setup_files[name.casefold()]
                for name in active_devices
                if name.casefold() in setup_files
            ]
        else:
            ordered_profiles = [setup_files[key] for key in sorted(setup_files)]

        bindings: list[ParsedAxisBinding] = []
        for device_name, path in ordered_profiles:
            try:
